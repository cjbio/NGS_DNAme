...
for(i in cont_names) {
  ...
  if(nrow(ttm) > 0) {
    ttm  <- ttm %>% dplyr::select(CpG,logFC,P.Value,`adj.P.Val`)
    tt <- ttm %>% 
...
sigYoungVsOld <- youngVsOld[youngVsOld$adj.P.Val < 0.05,]
write.csv(youngVsOld[youngVsOld$adj.P.Val < 0.05,], file = "~/DNAme/sig_DMP_youngVsOld.csv")
write.csv(sigYoungVsOld, file = "~/DNAme/sigDPM_youngVsOldLog2FC.csv")
